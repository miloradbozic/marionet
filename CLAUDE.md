...
npm run marionet -- replay --client opari set_product_ean --csv rows.csv --resume  # skip rows the last batch verified done
...
- **Auth loss is a pause, not a failure** (P40, the first P6 interference state): when a browser step fails, a cheap policy-gated probe checks whether the page is showing a password field. If yes, the run *pauses* — the CLI asks the operator to log back in **in their own browser** (P29) and then retries the exact same step verbatim. The check runs *before* the healer, because waking the healer against a login wall is the pathology: it happily re-grounds the step onto something that exists on the login page. A pause costs zero LLM calls; capped at 3 per replay so a wall that login can't fix doesn't loop. Warm-up gets the same treatment (an expired session at batch start is the human's 30-second fix, not an abort). Injectable via `onInterference`; absent = old behavior, so non-interactive callers are unchanged.
- **`--resume`** (`resume.ts`, the checkpointed slice of P17): re-running a partially failed `--csv` batch skips rows the *previous* batch run of that skill verified as done — the fix for the breaker's own aftermath, where re-running the CSV would re-submit the 213 rows that already succeeded. Row bookkeeping, not world-state-from-log: each skipped row passed its post-condition read-back when it originally ran; the log only remembers *which rows* that held for. Rows are matched by canonical param-JSON (a corrected value in the CSV makes it a different row, so it re-runs); `row_skipped_resume` markers are re-written into the new run's log so a resume of a resume still knows the full set. Probing the world per row instead was considered and rejected: compiled post-conditions are mostly page-local, so checking row N would require navigating to row N — the very work being skipped.
