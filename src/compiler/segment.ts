...
Return STRICT JSON only (no prose, no markdown fences):
{
  ...
  "segments": [
    { "name": "snake_case_verb_noun", "description": "one sentence",
      ...
      "postCondition": { "tool": "browser__extract", "args": { "selector": "...", "format": "value" }, "expectPattern": "regex" } }
  ],
...
- Every segment EXCEPT the last needs a postCondition: a read-only check (tool must be browser__extract, browser__wait_for, fs__read, or browser__eval) proving the segment reached its goal. PREFER browser__extract -- format "value" reads an input/select's current value, format "text" reads an element's visible text. It is read-only by construction, so a skill that verifies through it replays unattended even on clients where eval is gated to "ask"; only fall back to browser__eval for a check extract cannot express (e.g. reading location.hash). It MUST hold for any parameter values: never reference run-specific values like record UUIDs from the results; prefer structural checks (an element that only exists on the target page, location.hash shape, etc). It must also DISCRIMINATE: it must FAIL when the app is in the wrong state -- e.g. checking location.hash alone is worthless if a login redirect preserves the hash; check for an element that exists ONLY on the target page and NOT on a login/error page. And it must be GROUNDED: derive it only from facts visible in the steps' recorded results (page titles, URL shapes, selectors the later steps successfully acted on) -- never guess at page structure (e.g. do not assume an h1 exists unless a result mentions one; a wrong guess makes every future replay fail). The last segment's postCondition is ignored (the run's own verification is used).
