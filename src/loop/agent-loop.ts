...
Finishing with status "success" requires a verification: a read-only tool call the system executes to independently confirm the outcome (e.g. browser__extract with format "value" reading an input's current value, fs__read of a written file, a read-only shell command). Choose a check that reads durable state -- never a success toast or notification, those disappear and are unreliable. If verification fails you will get the result back and the run continues: fix the problem or finish with status "failure".

Prefer browser__extract over browser__eval for every read: extract with format "value" reads an input/textarea/select's current value, format "text" reads visible text. It is read-only by construction, so it stays allowed on clients where eval is gated for safety -- a verification written with extract replays unattended, one written with eval may stall waiting for a human. Reach for browser__eval only when a read genuinely needs JavaScript that extract cannot express.
...
- After executing a save (JS click or browser__submit_form), call finish_task immediately with a verification that re-reads the saved state deterministically (browser__extract with format "value" on the input). Do not check success toasts -- they disappear in seconds and are unreliable.
